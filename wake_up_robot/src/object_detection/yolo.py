...
class YoloModel:
    ...
    def get_best_detection(self, img_node, target):
        ...
        best_idx = next(
            i for i, (box, cls) in enumerate(zip(
                best_result.boxes.xyxy, best_result.boxes.cls
            )) if int(cls) == label_id
        )

        if hasattr(best_result, "masks") and best_result.masks is not None:
            mask = best_result.masks.data[best_idx].cpu().numpy()
        else:
            mask = None

        return best_det["box"], best_det["score"], mask
